...
public final class ATDWSearchTag extends SimpleTagSupport {
	...
	public void doTag() throws JspException, IOException {
		...
		final ATDWProductSearchParameters params = builder.setText(term).setCategory(cat).setState(state)
			.setRegion(region).setCity(city).setPage(page == null ? 1 : page)
			.setCount(count == null ? DEFAULT_COUNT : count).build();
