...
package com.australia.hibernate4.spring.fix;
 ...
 * with the same DataSource). This allows for mixing services which access
 * Hibernate and services which use plain JDBC (without being aware of
 * Hibernate)! Application code needs to stick to the same simple Connection
 * lookup pattern as with
 ...
 * transactions will just apply to the JDBC Connection, not to the Hibernate
 * Session and its cached objects. You can manually set the flag to "true" if
 * you want to use nested transactions for JDBC access code which participates
 * in Hibernate transactions (provided that your JDBC driver supports
...
public class HibernateTransactionManager extends AbstractPlatformTransactionManager implements
	 ...
	 * instead of {@link org.hibernate.SessionFactory#openSession()} (with a
	 * Spring
	 ...
	 * TransactionSynchronizationManager. It may also be used in combination
	 * with Spring's Open-Session-in-View support (using Spring's default
	 * {@link SpringSessionContext}), in which case it subtly differs from the
	 * Spring-managed Session mode: The pre-bound Session will <i>not</i>
	 * receive a {@code clear()} call (on rollback) or a {@code disconnect()}
	 * call (on transaction completion) in such a scenario; this is rather left
	...
	/**
	 * Set the bean name of a Hibernate entity interceptor that allows to
	...
	protected void doBegin(Object transaction, TransactionDefinition definition) {
		...
		try {
			if (txObject.getSessionHolder() == null || txObject.getSessionHolder().isSynchronizedWithTransaction()) {
				...
				Session newSession = (entityInterceptor != null ? getSessionFactory().withOptions()
					.interceptor(entityInterceptor).openSession() : getSessionFactory().openSession());
				if (logger.isDebugEnabled()) {
			...
			if (this.prepareConnection && isSameConnectionForEntireSession(session)) {
				...
				if (definition.getIsolationLevel() != TransactionDefinition.ISOLATION_DEFAULT) {
					// We should set a specific isolation level but are not
					// allowed to...
			...
			if (definition.isReadOnly() && txObject.isNewSession()) {
				// Just set to MANUAL in case of a new Session for this
				// transaction.
			...
			if (timeout != TransactionDefinition.TIMEOUT_DEFAULT) {
				// Use Hibernate's own transaction timeout mechanism on
				// Hibernate 3.1+
				// Applies to all statements, also to inserts, updates and
				// deletes!
			...
			// Register the Hibernate Session's JDBC Connection for the
			// DataSource, if set.
			if (getDataSource() != null) {
				Connection con = ((SessionImplementor) session).connection();
	...
	protected void doCleanupAfterCompletion(Object transaction) {
		...
		if (this.prepareConnection && session.isConnected() && isSameConnectionForEntireSession(session)) {
			// We're running with connection release mode "on_close": We're able
			// to reset
			// the isolation level and/or read-only flag of the JDBC Connection
			// here.
			// Else, we need to rely on the connection pool to perform proper
			// cleanup.
			try {
				Connection con = ((SessionImplementor) session).connection();
